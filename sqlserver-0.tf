resource "azurerm_public_ip" "sqlserver-0-pip" {
  name                         = "sqlserver-0-pip"
  location                     = "${azurerm_resource_group.res_group.location}"
  resource_group_name          = "${azurerm_resource_group.res_group.name}"
  public_ip_address_allocation = "static"
}

resource "azurerm_network_interface" "sqlserver-0-nic" {
  name                = "sqlserver-0-nic"
  resource_group_name = "${azurerm_resource_group.res_group.name}"
  location            = "${azurerm_resource_group.res_group.location}"
  ...
  ip_configuration {
    ...
    public_ip_address_id                    = "${azurerm_public_ip.sqlserver-0-pip.id}"
...
resource "azurerm_virtual_machine" "sqlserver-0-vm" {
  name                  = "sqlserver-0"
  resource_group_name   = "${azurerm_resource_group.res_group.name}"
  location              = "${azurerm_resource_group.res_group.location}"
  availability_set_id   = "${azurerm_availability_set.avail_set2.id}"
  network_interface_ids = ["${azurerm_network_interface.sqlserver-0-nic.id}"]
